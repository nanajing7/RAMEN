...
## Experiment C — Vary K ({1, 2, 3})
# ============================================================


# ============================================================
# Experiment C: Computation Time — Vary K (latent dimension)
#
...
# ------------------------------------------------------------
# 2. DGP generator  (K, TT, M are arguments)
# ------------------------------------------------------------
generate_panel <- function(N, M, TT, K, seed = 42) {
  set.seed(seed)
  years      <- 2001:(2000 + TT)
  ...
  for (tt in seq_len(TT)) {
    ...
    # Latent part: generalised to K dimensions (works for K=1,2,3,4,...)
    r_idx <- match(base_grid$node_row, row_ids)
    c_idx <- match(base_grid$node_col, col_ids)
    latent_part <- rowSums(U_list[[tt]][r_idx, , drop = FALSE] *
                             V_list[[tt]][c_idx, , drop = FALSE])
  ...
  list(N=N, M=M, TT=TT, K=K, years=years,
...
time_ramen_boot <- function(dat, B = B_boot) {
  ...
  for (tt in seq_len(TT)) {
    ...
    r_names <- rownames(res_tt$U)
    c_names <- names(res_tt$beta)
...
# AMEN: T independent ame(R=K) calls, total time summed
...
# ------------------------------------------------------------
# 4. Experiment C: loop over K
# ------------------------------------------------------------
cat("\n============================================================\n")
cat(sprintf("Experiment C: Vary K  (N=%d, T=%d, B=%d, nscan_amen=%d)\n",
            N_fixed, TT_fixed, B_boot, nscan_amen))
cat(sprintf("  ramen: %d\u00d7%d (joint)  |  AMEN: %d\u00d7%d (\u00d7T)\n",
...
for (K_val in K_grid) {
  ...
  dat_ramen <- generate_panel(N = N_fixed, M = M_ramen_fixed,
                              TT = TT_fixed, K = K_val, seed = 42)
  dat_amen  <- generate_panel(N = N_fixed, M = M_amen_fixed,
                              TT = TT_fixed, K = K_val, seed = 42)

  # ramen + bootstrap (all T periods)
  t_rb <- tryCatch(
    median_elapsed(time_ramen_boot, dat_ramen),
    error = function(e) { cat("  ramen boot FAILED:", conditionMessage(e), "\n"); NA }
  )
  cat(sprintf("  ramen+boot (K=%d): %.2f sec  (B=%d, T=%d)\n",
              K_val, t_rb, B_boot, TT_fixed))
  ...
  tryCatch({
    ...
    ok  <- TRUE
    cat(sprintf("  AMEN       (K=%d): %.2f sec  (%d \u00d7 nscan=%d)\n",
                K_val, t_a, TT_fixed, nscan_amen))
  }, error = function(e) {
    cat(sprintf("  AMEN: FAILED — %s\n", conditionMessage(e)))
  })

  timing_C <- rbind(timing_C, data.frame(
    K = K_val, t_ramen_boot = t_rb, t_amen = t_a, amen_ok = ok
  ))
}
...
cat("\n── Summary ──\n")
print(timing_C %>% dplyr::select(K, t_ramen_boot, t_amen, speedup), digits = 3)
...
plot_df <- timing_C %>%
  ...
  mutate(
    method = dplyr::recode(method,
                           ...
                           t_amen       = sprintf("AMEN (T\u00d7nscan=%d)", nscan_amen)
    ),
    method = factor(method, levels = c(
      sprintf("ramen (point est. + bootstrap B=%d)", B_boot),
      sprintf("AMEN (T\u00d7nscan=%d)", nscan_amen)
    ))
  ) %>%
  filter(!is.na(elapsed))

p_line <- ggplot(plot_df, aes(x = K, y = elapsed,
  ...
  labs(
    ...
    title    = sprintf("Computation time vs latent dimension  (N=%d, T=%d)", N_fixed, TT_fixed),
...
p_bar <- ggplot(plot_df, aes(x = factor(K), y = elapsed, fill = method)) +
  geom_col(position = "dodge", width = 0.6) +
  scale_fill_manual(values = c("steelblue", "tomato")) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "bottom") +
  labs(x = "Latent dimension K", y = "Elapsed time (seconds)", fill = NULL,
...
save_path <- file.path(save_dir,
                       sprintf("part3c.RData", timestamp))
save(timing_C, K_grid, N_fixed, TT_fixed, lambda_val, gamma_val,
     nscan_amen, burn_amen, B_boot, n_time_reps,
     file = save_path)
cat(sprintf("\nResults saved to:\n  %s\n", save_path))

for (pp in list(list(p_line, "line"), list(p_bar, "bar"))) {
  path_p <- file.path(save_dir,
                      sprintf("part3c.png", pp[[2]], timestamp))
